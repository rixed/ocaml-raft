...
struct
    ...
    type leader_t = (Address.t, peer_info) Hashtbl.t
    ...
    struct
        ...
        type ret = State of int * Command.state (* to a ChangeState only *)
                 | Redirect of Address.t (* to a ChangeState only *)
    ...
    struct
        type t = server
        let print fmt t =
            Printf.fprintf fmt "%20s@%s, term=%d" (string_of_state t.info.state) (Address.to_string t.info.host) t.info.current_term
            ...
            struct
                type arg =
                    { term : term_id (* candidate's term *) ;
                      candidate_id : Address.t (* candidate requesting vote *) ;
            ...
            struct
                type arg =
                    { term : term_id (* leader's term *) ;
                      leader_id : Address.t (* so follower can redirect clients *) ;
        ...
        let call_server t peer arg k =
            L.debug "%a: Calling %s for %a" print t (Address.to_string peer) print_arg arg ;
            RPC_Servers.call peer arg (fun res ->
                ...
                | Timeout ->
                    L.error "%a: Timeouting call %s for %a" print t (Address.to_string peer) print_arg arg
                | Err err ->
        ...
        let answer ?dest ?reason t success =
            L.debug "%a: Answering %s%s%s"
                ...
                (match dest with Some d -> " to "^ Address.to_string d | None -> "")
        ...
        let answer_request_vote t arg =
            ...
            let grant, reason =
                ...
                match t.info.voted_for with Some guy when guy <> arg.candidate_id ->
                      false, "I vote for "^(Address.to_string guy)
                | _ ->
                    ...
                    call_server t peer (RPC_Server_Types.AppendEntries arg) (function
                        | Ok res ->
                            ...
                            if res.success then (
                                ...
                                L.debug "%a: Received ack from %s, next_index will be %d" print t (Address.to_string peer) info.next_index
                            ) else (
                                (* The peer could find where to attach that *)
                                info.next_index <- info.next_index - 1 ;
                                L.debug "%a: Received nack from %s, next_index will be %d" print t (Address.to_string peer) info.next_index
                                (* TODO: retry faster *)
                            )
                        | Timeout | Err _ ->
                            L.error "%a: Cannot append entries to %s" print t (Address.to_string peer) ;
                            (* TODO: close? *))
                )
            ) t.info.peers
        ...
        let pub_of_raft (h : Address.t) = { h with port = h.port - 1 }
        let raft_of_pub (h : Address.t) = { h with port = h.port + 1 }
        ...
        let serve pub_host peers apply =
            ...
            RPC_ClientServers.serve pub_host (fun write cmd ->
                ...
                | ChangeState command ->
                    ...
                    | _ ->
                        L.debug "%a: Received a command while I'm not the leader, redirecting to %s" print t (Address.to_string t.info.last_leader) ;
                        redirect t write)
                | QueryInfo ->
    ...
    struct
        type t = { name : string ;
                   servers : Address.t array ; (* List of all known servers *)
                   mutable leader : Address.t (* the one believed to be good *) }
        ...
        let info ?(timeout=0.5) t host k =
            RPC_ClientServers.call ~timeout host QueryInfo (function
                | Ok (Info info) -> k info
                | Ok _ ->
                    L.debug "%a: server %s answering with state not infos" print t (Address.to_string host)
                | Timeout ->
                    L.error "%a: Timeouting getinfo to %s" print t (Address.to_string host)
                | Err x ->
                    L.error "%a: Can't get infos from %s: %s" print t (Address.to_string host) x)
        ...
        let call ?(timeout=0.5) t x k =
            ...
            let rec retry nb_fast_try nb_slow_try =
                L.debug "%a: Sending %a to %s after %d fast, %d slow retries" print t Command.print x (Address.to_string t.leader) nb_fast_try nb_slow_try ;
                if nb_fast_try > max_fast_try then (
                    ...
                    RPC_ClientServers.call ~timeout t.leader (ChangeState x) (function
                        | Ok (State (log_index, state)) ->
                            ...
                            k state
                        | Ok (Redirect leader') ->
                            L.debug "%a: Was told by %s to redirect to %s" print t (Address.to_string sent_to) (Address.to_string leader') ;
                            t.leader <-
                                ...
                                if t.leader <> sent_to then t.leader else (
                                    ...
                                    L.debug "%a: Will redirect to %s instead" print t (Address.to_string l) ;
                                    l
                                ) ;
                            retry (nb_fast_try+1) nb_slow_try
                        | Ok (Info s) ->
                            L.debug "%a: server %s sending its status out of the blue?" print t (Address.to_string s.host) ;
                            assert false (* TODO *)
                        | Timeout ->
                            L.error "%a: Timeouting query %a to %s" print t Command.print x (Address.to_string sent_to) ;
                            retry (nb_fast_try+1) nb_slow_try
                        | Err x ->
                            L.error "%a: Can't get state from %s: %s" print t (Address.to_string t.leader) x ;
                            (* try another server? *)
